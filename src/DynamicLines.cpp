  ...
  Real *prPos = static_cast<Real*>(vbuf->lock(HardwareBuffer::HBL_DISCARD));
  {
   ...
   for(int i = 0; i < size; i++)
   {
      ...
      *prPos++ = mPArray[i].z;
      ...
      if(mPArray[i].x < vaabMin.x)
         vaabMin.x = mPArray[i].x;
      if(mPArray[i].y < vaabMin.y)
         vaabMin.y = mPArray[i].y;
      if(mPArray[i].z < vaabMin.z)
         vaabMin.z = mPArray[i].z;
 
      if(mPArray[i].x > vaabMax.x)
         vaabMax.x = mPArray[i].x;
      if(mPArray[i].y > vaabMax.y)
         vaabMax.y = mPArray[i].y;
      if(mPArray[i].z > vaabMax.z)
         vaabMax.z = mPArray[i].z;
   }
  }
  vbuf->unlock();
